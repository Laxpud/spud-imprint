...
Spud Imprint 是一个批量照片加印与水印工具，用于把照片、画布布局、元数据文字和导出规则组合成可复用的处理流程。AI 代理应把本文件当作协作入口，项目方向和技术决策以 `docs/architecture.md` 为准。
...
- 修改架构、模块职责、CLI/核心边界、配置流或未来 GUI 集成方式时，同步检查并按需更新 `AGENTS.md` 的文档入口、模块路由和行为边界，以及 `docs/architecture.md`。
- 涉及 GUI、渲染核心迁移或重大技术选型时，先阅读并同步更新 `docs/architecture.md`。
