...
当前状态更新于 2026-06-08。下次继续开发时，优先从第 11 项交互预览原型开始。
...
当前状态：已新增 `gui/` Tauri 2 + React/TypeScript 技术原型。第一版通过桌面界面选择输入目录、输出目录和 TOML 配置文件，由 Rust 后端调用现有 Python CLI，并把命令、退出码、标准输出和标准错误显示在日志面板中。详细说明见 `docs/gui-prototype.md`。
