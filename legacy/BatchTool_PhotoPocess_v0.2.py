...
# 在 add_metadata_to_image 函数中实现动态字体计算
def add_metadata_to_image(image, metadata, fields_to_draw, 
                         position_preset="center middle",
                         margin_percent=0.05,
                         text_style=None,
                         base_resolution=(1920, 1080),  # 新增：基准分辨率参数化[1,5](@ref)
                         scale_mode="area",  # 缩放模式：width/area
                         min_font_size=8):    # 新增：最小字体限制
    ...
    if scale_mode == "width":
        ...
        # 按面积比例缩放（更均衡适应长宽比）[1](@ref)
        base_area = base_width * base_height
        current_area = width * height
        scale_factor = (current_area / base_area) ** 0.5  # 面积平方根比例
    
    # 应用缩放并添加最小字体限制
    dynamic_font_size = max(
        min_font_size,  # 确保不小于最小值
        int(text_style.font_size * scale_factor)
    )
    
    # 使用动态字体
    try:
        font = ImageFont.truetype(text_style.font_name, dynamic_font_size)
    except IOError:
        font = ImageFont.load_default()
    
    # 动态计算行高（基于实际字体大小）[8](@ref)
    line_height = int(dynamic_font_size * text_style.line_spacing)
...
def calculate_text_positions(image_size, text_lines, preset, 
    ...
    该函数将元数据字段以文本形式添加到图像指定位置，支持自定义位置、样式和布局

    Args:
        image (PIL.Image): 要处理的PIL图像对象
        metadata (dict): 分类元数据字典（来自get_categorized_metadata）
        fields_to_draw (list): 要显示的字段名列表（如["DateTimeOriginal", "FNumber"]）
        position_preset (str, optional): 文本块位置预设，格式"水平方向 垂直方向"
            水平方向: "left"/"center"/"right"
            垂直方向: "top"/"middle"/"bottom"
            默认: "center middle"
        margin_percent (float, optional): 边距占图像尺寸的百分比（0-1）
            默认: 0.05（即5%边距）
        text_style (TextStylePreset, optional): 文本样式配置对象
            如未提供则使用默认样式

    Returns:
        PIL.Image: 添加元数据文本后的图像对象

    Example:
        >>> img = Image.open("photo.jpg")
        >>> metadata = get_categorized_metadata("photo.jpg")
        >>> add_metadata_to_image(
                img,
                metadata,
                ["DateTimeOriginal"],
                position_preset="bottom right",
                text_style=TextStylePreset(font_size=20)
            )
    """        
    width, height = image_size
    parts = preset.lower().split()
    ...
    # 使用传入的行高计算文本块高度
    text_block_height = len(text_lines) * line_height  # 修改：直接使用动态行高[8](@ref)
